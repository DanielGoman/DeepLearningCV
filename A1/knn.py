...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i, train_sample in enumerate(x_train):
    for j, test_sample in enumerate(x_test):
      dists[i, j] = torch.sqrt(torch.sum((torch.flatten(train_sample) - torch.flatten(test_sample))**2))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  flat_test = torch.flatten(x_test, start_dim=1)
  for i, train_sample in enumerate(x_train):
    dists[i, :] = torch.sqrt(torch.sum((flat_test - torch.flatten(train_sample))**2, dim=1))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  # Replace "pass" statement with your code
  print(x_train.shape)
  flat_train = torch.flatten(x_train, start_dim=1)
  flat_test = torch.flatten(x_test, start_dim=1)

  dists = (dists.t() + (flat_train**2).sum(dim=1)).t()
  dists += (flat_test**2).sum(dim=1)
  dists -= 2 * torch.matmul(flat_train, flat_test.t())
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  _, top_k_idx = torch.topk(dists, k=k, dim=0, largest=False)
  print(y_train.shape)
  for i, top_k in enumerate(y_train[top_k_idx].t()):
    counter = Counter(top_k.numpy())
    max_count = None
    max_keys = []
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  x_train_folds = torch.stack(torch.chunk(x_train, chunks=num_folds, dim=0))
  y_train_folds = torch.stack(torch.chunk(y_train, chunks=num_folds, dim=0))
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
  ...
  for k in k_choices:
    ...
    for fold_num in range(num_folds):
      if fold_num + 1 != num_folds:
        x_folds = torch.cat((x_train_folds[:fold_num], x_train_folds[fold_num+1:]))
        y_folds = torch.cat((y_train_folds[:fold_num], y_train_folds[fold_num+1:]))
      else:
        x_folds = x_train_folds[:fold_num]
        y_folds = y_train_folds[:fold_num]
        
      x_folds = x_folds.view(-1, x_folds.shape[2], x_folds.shape[3], x_folds.shape[4])
      y_folds = y_folds.view(-1)
      x_val_fold = x_train_folds[fold_num]
      y_val_fold = y_train_folds[fold_num]

      knn_classifier = KnnClassifier(x_folds, y_folds)
      accuracy = knn_classifier.check_accuracy(x_val_fold, y_val_fold, k=k)
...
def knn_get_best_k(k_to_accuracies):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  mean_acc = {k: np.mean(acc_list) for k, acc_list in k_to_accuracies.items()}
